...
export default async function xmlZipToJson(data: Blob): Promise<Score | undefined> {
	try {
		...
		dispatchEvent(new ErrorEvent("error", { error: {message: "Kon nummer niet converteren", retryable: true, error: error} }));
	}
}
